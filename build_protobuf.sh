...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-8.1" || "$ID-$VERSION_ID" == "rhel-8.2"|| "$ID-$VERSION_ID" == "rhel-8.3" || "$ID-$VERSION_ID" == "ubuntu-20.04"|| "$ID-$VERSION_ID" == "ubuntu-22.04" || "$ID-$VERSION_ID" == "sles-15.2" ]]; then
        cd "$CURDIR"
